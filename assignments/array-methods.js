...
let allCaps = runners.map(
  element => `${element.first_name.toUpperCase()} ${element.last_name}`
);
console.log(allCaps);
...
let ticketPriceTotal = runners.reduce(
  (acc, element) => (acc += element.donation),
  0
);
...
// Problem 1
// make an object that counts the instances of each size shirt needed
let shirts = runners.reduce((acc, element) => {
  if (!acc[element.shirt_size]) {
    acc[element.shirt_size] = 1;
  } else {
    acc[element.shirt_size]++;
  }
  return acc;
}, {});

// Problem 2
// make a new array of all the company names
let companies = runners.map(element => element.company_name);
console.log(companies);

// Problem 3
// make an array of people who donated $100 or more
let highDonation = runners.filter(element => element.donation >= 100);
